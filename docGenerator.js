...
async function generateDoc(submission, settings) {
  ...
  const dataRows = await Promise.all(items.map(async (item, idx) => {
               ...
               const shading = { fill: bg, type: ShadingType.CLEAR };
                                                   ...
                                                   // Kolom Info: tampilkan QR code jika ada link
                                                   let infoCell;
               if (item.link) {
  ...
  const docChildren = [
               ...
               new Paragraph({
                              spacing: { after: 0, line: 276 },
                              children: [new TextRun({ text: `Bersama ini kami ${companyName} yang bergerak dibidang ${companyTagline.toLowerCase()} bermaksud mengajukan penawaran produk sebagai berikut :`, font: 'Times New Roman', size: 24 })]
               }),
               emptyPara(),
               produkTable,
               emptyPara(),
