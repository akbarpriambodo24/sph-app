...
router.post('/', requireLogin, (req, res) => {
  const {
    client_title, client_name, client_address, client_city,
    items, ppn_included, ongkir_included, notes, lampiran
  } = req.body;
  ...
  const stmt = db.prepare(`
    ...
    (client_title, client_name, client_address, client_city, items, ppn_included, ongkir_included, notes, lampiran, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
// GET /api/submissions/:id/download - download DOCX (hanya jika approved)
router.get('/:id/download', requireLogin, async (req, res) => {
  ...
  // Staff hanya bisa download miliknya
  if (req.session.user.role !== 'admin' && row.created_by !== req.session.user.id) {
    return res.status(403).json({ error: 'Akses ditolak' });
  }
...
router.get('/:id/download/pdf', requireLogin, async (req, res) => {
  ...
  try {
    ...
    const tmpHtml = path.join(os.tmpdir(), `sph_${stamp}.html`);
    const tmpPdf  = path.join(os.tmpdir(), `sph_${stamp}.pdf`);
    ...
    execFile('wkhtmltopdf', [
      ...
      try {
        ...
        res.status(500).json({ error: 'Gagal membaca PDF' });
      }
    });
  } catch (err) {
...
// POST /api/submissions/meta/upload/:type - upload logo atau tanda tangan
